...
### `33_builtins/` — новые встроенные функции ✅ ГОТОВО
- `01_min-max` (Go 1.21) — любой набор аргументов, любой упорядочиваемый тип, приём clamp ✅
- `02_clear` (Go 1.21) — очистка map (удаляет пары) и срезов (обнуляет, длину не меняет) ✅
- Один модуль (`example.com/builtins`, `go 1.21`). gofmt+vet чисто, оба запускаются.
